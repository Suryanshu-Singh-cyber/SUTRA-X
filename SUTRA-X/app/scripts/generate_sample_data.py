...
def generate_sample_data():
    ...
    print(f"✅ Generated {len(df_cases)} cases")
    
    # Generate persons with Indian names
    first_names = ['Raj', 'Amit', 'Priya', 'Suresh', 'Anita', 'Vikram', 'Neha', 'Rahul', 'Sunita', 'Mohan',
                   'Geeta', 'Arjun', 'Kavita', 'Deepak', 'Anjali', 'Sanjay', 'Meera', 'Ravi', 'Pooja', 'Kumar']
    last_names = ['Sharma', 'Singh', 'Patel', 'Reddy', 'Rao', 'Joshi', 'Gupta', 'Verma', 'Kumar', 'Nair']
    ...
    for i in range(50):
        person = {
            ...
            'alias': f'Alias {random.randint(1,5)}' if random.random() > 0.6 else '',
            'gender': random.choice(['M', 'F']),
            'age': random.randint(18, 65),
            'dob': (datetime.now() - timedelta(days=random.randint(365*18, 365*65))).strftime('%Y-%m-%d'),
            ...
            'pan': f'{random.choice("ABCDEFGHIJ")}{random.choice("ABCDEFGHIJ")}{random.choice("ABCDEFGHIJ")}{random.randint(1000,9999)}{random.choice("ABCDEFGHIJ")}',
    ...
    print(f"✅ Generated {len(df_persons)} persons")
    ...
    for i in range(30):
        phone = {
            ...
            'registered_at': (datetime.now() - timedelta(days=random.randint(1, 1000))).strftime('%Y-%m-%d %H:%M:%S'),
            'last_active': (datetime.now() - timedelta(days=random.randint(1, 30))).strftime('%Y-%m-%d %H:%M:%S'),
    ...
    print(f"✅ Generated {len(df_phones)} phones")
    ...
    for i in range(100):
        call = {
            ...
            'call_time': (datetime.now() - timedelta(days=random.randint(1, 30), hours=random.randint(0, 23))).strftime('%Y-%m-%d %H:%M:%S'),
    ...
    print(f"✅ Generated {len(df_cdr)} CDR records")
    ...
    for i in range(80):
        transaction = {
            ...
            'currency': 'INR',
            'transaction_date': (datetime.now() - timedelta(days=random.randint(1, 90))).strftime('%Y-%m-%d %H:%M:%S'),
    ...
    # Generate vehicles
    vehicle_prefixes = ['MH', 'DL', 'KA', 'TN', 'TS', 'GJ', 'UP', 'WB']
    vehicles = []
    for i in range(20):
        vehicle = {
            'vehicle_id': f'V-{i+1:04d}',
            'registration_number': f'{random.choice(vehicle_prefixes)}{random.randint(1,99)} {random.choice(["AB","CD","EF","GH","IJ"])}{random.randint(1000,9999)}',
            'owner_id': random.choice(df_persons['person_id'].tolist()),
            'owner_type': 'PERSON',
    ...
    print(f"✅ Generated {len(df_vehicles)} vehicles")
    ...
    for i in range(40):
        report = {
            ...
            'date_reported': (datetime.now() - timedelta(days=random.randint(1, 60))).strftime('%Y-%m-%d %H:%M:%S'),
            'priority': random.choice(['high', 'medium', 'low']),
    ...
    print(f"✅ Generated {len(df_reports)} reports")
    
    print("\n🎉 Sample data generation complete!")
    print(f"📁 Data saved to: {data_dir}")
    print(f"Total records: {sum([len(df_cases), len(df_persons), len(df_phones), len(df_cdr), len(df_transactions), len(df_vehicles), len(df_locations), len(df_reports)])}")
