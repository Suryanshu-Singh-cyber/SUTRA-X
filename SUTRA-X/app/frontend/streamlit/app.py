...
# ============================================================================
# LOAD GRAPH FROM CSV FILES - NEW FEATURE
# ============================================================================

def load_graph_from_csvs():
    """Load graph from CSV files (entities.csv + relationships.csv)"""
    try:
        entities_file = Path("entities.csv")
        relationships_file = Path("relationships.csv")
        
        if not entities_file.exists():
            st.warning("⚠️ entities.csv not found! Run generate_graph_csv.py first.")
            return False
        
        if not relationships_file.exists():
            st.warning("⚠️ relationships.csv not found! Run generate_graph_csv.py first.")
            return False
        
        with st.spinner("📊 Loading graph from CSV files..."):
            # Load data
            entities_df = pd.read_csv(entities_file)
            relationships_df = pd.read_csv(relationships_file)
            
            st.info(f"📊 Found {len(entities_df):,} entities and {len(relationships_df):,} relationships")
...
with st.sidebar:
    ...
    # Navigation - Simplified "App" section
    st.markdown("### 📌 App")
    nav_pages = [
        "📊 Dashboard",
        "🌐 Network Graph",
    ...
    # Data Controls - Updated with CSV Loader
    st.markdown("### 📊 Data")
    
    # Load from CSV button
    if st.button("📂 Load CSV Graph", use_container_width=True):
        with st.spinner("Loading..."):
            if load_graph_from_csvs():
                add_audit_log("load_csv", "CSV Graph", "Loaded entities.csv + relationships.csv")
                st.rerun()
